module.exports = (username, bookingDetails) => {
  const { listing, checkIn, checkOut, guests, totalPrice, cancellationReason } =
    bookingDetails;
...
<div style="font-family: 'Plus Jakarta Sans', Arial, sans-serif; max-width: 600px; margin: 0 auto;">
  ...
  <div style="background: linear-gradient(135deg, #6c757d 0%, #868e96 100%); padding: 30px; text-align: center; border-radius: 10px 10px 0 0;">
    ...
    <p style="color: white; margin: 10px 0 0 0; font-weight: 500;">Booking Cancelled</p>
  </div>
  ...
  <div style="padding: 30px; background: #f9f9f9; border-radius: 0 0 10px 10px;">
    ...
    <div style="background: white; padding: 20px; margin: 20px 0; border-radius: 8px; border-left: 4px solid #6c757d;">
      <h3 style="color: #6c757d; margin-top: 0;">${listing.title}</h3>
      ...
      <p style="color: #6c757d; margin: 15px 0 0 0; font-size: 18px;"><strong>Cancelled Amount: ₹${totalPrice.toLocaleString(
        "en-IN"
      )}</strong></p>
      ${
        cancellationReason
          ? `<p style="color: #666; margin: 10px 0 0 0;"><strong>Reason:</strong> ${cancellationReason}</p>`
          : ""
      }
    </div>
    
    <p style="color: #666; line-height: 1.6;">
      If you have any questions about this cancellation or need assistance with rebooking, please don't hesitate to contact our support team.
    </p>
